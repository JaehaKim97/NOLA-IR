...
from datasets.degradation import random_add_gaussian_noise_pt, random_add_poisson_noise_pt
...
class RealESRGANBatchTransform(BatchTransform):
    ...
    def __call__(self, batch: Dict[str, torch.Tensor | List[str]]) -> Dict[str, torch.Tensor | List[str]]:
        ...
        if updown_type == "up":
            ...
            if self.down_only:
                scale = np.random.uniform(self.resize_range2[0], self.resize_range2[1])
            else:
