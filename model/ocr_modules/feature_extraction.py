...
class VGG_FeatureExtractor(nn.Module):
    ...
    def __init__(self, input_channel, output_channel=512):
        ...
        self.ConvNet = nn.Sequential(
            ...
            nn.Conv2d(self.output_channel[0], self.output_channel[1], 3, 1, 1),  # (i == 2)
            nn.ReLU(True),
            nn.MaxPool2d(2, 2),  # 128x8x25
            nn.Conv2d(self.output_channel[1], self.output_channel[2], 3, 1, 1), nn.ReLU(True),  # 256x8x25
            nn.Conv2d(self.output_channel[2], self.output_channel[2], 3, 1, 1),  # (i == 7)
            nn.ReLU(True),
            nn.MaxPool2d((2, 1), (2, 1)),  # 256x4x25
            nn.Conv2d(self.output_channel[2], self.output_channel[3], 3, 1, 1, bias=False),
            nn.BatchNorm2d(self.output_channel[3]), nn.ReLU(True),  # 512x4x25
            nn.Conv2d(self.output_channel[3], self.output_channel[3], 3, 1, 1, bias=False),  # (i == 14)
...
class RCNN_FeatureExtractor(nn.Module):
    ...
    def forward(self, input):
        return self.ConvNet(input), None
...
class ResNet_FeatureExtractor(nn.Module):
    ...
    def forward(self, input):
        ...
        # Modified
        output, feats = self.ConvNet(input)
        return output, feats
