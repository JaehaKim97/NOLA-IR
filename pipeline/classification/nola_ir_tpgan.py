...
from pipeline.classification.nola_ir import NOLAIRPipeline
...
class NOLAIRTPGANPipeline(NOLAIRPipeline):
    ...
    def prepare_all(self):
        attrs = ["res_model", "vae", "text_encoder", "unet", "unet_ref",
                 "cls_model", "cls_hq_model", "disc", "val_dataloader"]
        if not self.args.eval_only:
            attrs += ["opt_gen", "sch_gen", "opt_cls_model", "sch_cls_model",
    ...
    def train(self):
        self.loss_records = dict(HLF=[], REG=[], CLS=[], ADV=[], FM=[], DISC=[], D_REAL=[], D_FAKE=[])
            ...
            for batch in self.train_dataloader:
                ...
                with self.accelerator.autocast():
                    ...
                    with torch.no_grad():
                        ...
                        _, feat_res_ref = self.cls_model(res_ref, return_feat=True)
                        _, feat_hq_res_ref = self.cls_hq_model(res_ref, return_feat=True)
                ...
                with self.accelerator.autocast():
                    ...
                    loss_disc_real, real_logits = self.disc(self.gt[:bh], for_real=True, return_logits=True)
                ...
                pbar.set_description(
                    ...
                    # f"CLS: {loss_cls.item():.3f}, FM: {loss_fm.item():.3f}"
