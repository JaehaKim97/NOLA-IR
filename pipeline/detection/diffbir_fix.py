...
from pipeline.detection.diffbir_only import DiffBIROnlyPipeline
from utils.detection import CocoEvaluator
from utils.common import calculate_psnr_pt, suppress_stdout, wavelet_reconstruction
...
class DiffBIRFixPipeline(DiffBIROnlyPipeline):
    ...
    def prepare_all(self):
        attrs = ["res_model", "vae", "text_encoder", "cldm", "det_model", "det_hq_model", "val_dataloader"]
        if not self.args.eval_only:
        ...
        while self.global_step < self.max_steps:
            ...
            for batch in self.train_dataloader:
                ...
                with self.accelerator.autocast():
                    ...
                    _, loss_dict = self.det_model(res_list + self.gt_list[b:], self.annot)
                    loss_det = sum(loss_value for loss_value in loss_dict.values())
                ...
                pbar.set_description(
                    f"Epoch: {self.epoch:04d}, Steps: {self.global_step:07d}, DET: {loss_det.item():.3f}"
                )
                ...
                if self.global_step % self.cfg.train.image_every == 0 or (self.args.debug):
                    self.log_images(img_dict={"pre_restored": pre_res, "restored": res}, N=8)
