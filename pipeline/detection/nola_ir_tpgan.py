...
class NOLAIRTPGANPipeline(NOLAIRPipeline):
    ...
    def prepare_all(self):
        attrs = ["res_model", "vae", "text_encoder", "unet", "unet_ref",
                 "det_model", "det_hq_model", "disc", "val_dataloader"]
        if not self.args.eval_only:
            attrs += ["opt_gen", "sch_gen", "opt_det_model", "sch_det_model",
    ...
    def train(self):
        self.loss_records = dict(HLF=[], REG=[], DET=[], ADV=[], FM=[], DISC=[], D_REAL=[], D_FAKE=[])
        ...
        while self.global_step < self.max_steps:
            ...
            for batch in self.train_dataloader:
                ...
                b = self.gt.size(0)
                bh = b // 2
                B = self.cfg.train.get("effective_batch_size", b) # lower this if OOM occurs
                ...
                with self.accelerator.autocast():
                    ...
                    with torch.no_grad():
                        z_res_ref = self.partial_diff_and_sample(z_pre_res[:B], z_pre_res[:B], c_txt[:B],
                                                                 t[:B], use_ref=True, mode="train")
                        res_ref = wavelet_reconstruction(self.decode_image(z_res_ref), pre_res[:B])
                        res_ref_list = self.batch2list(res_ref, self.gt_list[:B])
                        _, _, feat_res_ref = self.det_model(res_ref_list, return_feat=True)
                        _, _, feat_hq_res_ref = self.det_hq_model(res_ref_list, return_feat=True)

                    z_res = self.partial_diff_and_sample(z_pre_res[:B], z_pre_res[:B], c_txt[:B], t[:B], mode="train")
                    res = wavelet_reconstruction(self.decode_image(z_res), pre_res[:B])
                    res_list = self.batch2list(res, self.gt_list[:B])

                    # hlf loss
                    _, _, feat_gt = self.det_model(self.gt_list[:B], return_feat=True)
                    _, _, feat_res = self.det_model(res_list, return_feat=True)
                    with torch.no_grad():
                        _, _, feat_hq_gt = self.det_hq_model(self.gt_list[:B], return_feat=True)
                ...
                with self.accelerator.autocast():
                    ...
                    _, loss_dict, feat_mix = self.det_model(res_list[:bh] + self.gt_list[bh:],
                                                            self.annot, return_feat=True)
                ...
                pbar.set_description(
                    ...
                    # f"DET: {loss_det.item():.3f}, FM: {loss_fm.item():.3f}"
                ...
                if self.global_step % self.cfg.train.image_every == 0 or (self.args.debug):
                    self.log_images(img_dict={"pre_restored": pre_res, 'restored_ref': res_ref, "restored": res}, N=4)
